...
def reshape_and_pivot(df):
    reshaped_df = df.melt(id_vars='The column to hold constant when reshaping')
    pivoted_df = df.pivot_table(index='The index column when pivoting', columns='The columns to pivot into new columns')
...
def main():
    ...
    print('\n--- Reshaped and Pivoted Data ---')
    reshaped_df, pivoted_df = reshape_and_pivot(df)
