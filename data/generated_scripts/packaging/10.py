...
assert version.parse("3.9") == version.parse(".".join(map(str, torch.__version__.split(".")[:-1])))

class Warehouse:
    def __init__(self):
        self.state = {'Recyclability': 0, 'Efficiency in Shipping & Handling': 0, 'Physical Protection': 0}

        self.state['Marketing'] = 1
        self.state['Theft Reduction'] = 1
        self.state['Child-resistant'] = 1
...
class TestWarehouse(unittest.TestCase):
    ...
    def test_PytorchModel_init(self):
        model = PytorchModel()
        self.assertTrue(isinstance(model, nn.Module))

    def test_PytorchModel_forward_pass(self):
        model = PytorchModel()
        x = torch.rand((10, 3))
        out = model(x)
        self.assertEqual(out.shape, torch.Size([10, 3]))

    def test_PytorchDataset_init(self):
        states = torch.rand((10, 3))
        ds = PytorchDataset(states)
