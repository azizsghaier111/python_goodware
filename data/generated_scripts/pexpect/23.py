...
class TestShellProcess(unittest.TestCase):
    ...
    def test_multiple_commands(self):
        ...
        for cmd in cmds:
            self.assertTrue(self.process.send_and_read(cmd).strip().endswith(cmd.split()[-1]))

    def test_pattern_search(self):
        pattern = "[0-9]"
        output = self.process.send_and_expect("echo 1", pattern)
        self.assertTrue(output)
     
    def test_spaces_in_command(self):
        self.assertTrue(self.process.send_and_read("echo 'Hello World'").strip().endswith('Hello World'))
        
    def test_long_command(self):
        cmd = "echo 'This is a long command to test the capability of handling long commands'"
        self.assertTrue(self.process.send_and_read(cmd).strip().endswith('handling long commands'))

    def test_unexpected_output(self):   
        self.assertFalse(self.process.send_and_expect("echo 3", "This output is not expected"))
