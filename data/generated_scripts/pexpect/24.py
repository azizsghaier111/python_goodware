...
class ShellProcessTest(unittest.TestCase):
    ...
    def test_shell_process_read(self, mock_read):
        mock_read.return_value = 'mock output'
        shell_process = ShellProcess()
        result = shell_process.read()
    ...
    def test_shell_process_stop(self, mock_stop):
        ...
        self.assertIsNone(shell_process.command)

    @patch('pexpect.spawn')
    def test_shell_process_raise_exception(self, mock_spawn):
        mock_spawn.side_effect = ExceptionPexpect('Error occurred')
        shell_process = ShellProcess()
        self.assertRaises(ShellProcessException, shell_process.start)
