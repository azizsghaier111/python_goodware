...
def add_or_modify_exif_data(file_path, key, value):
    check_file(file_path)
    exif_dict = read_exif(file_path)

    # '0th' IFD is used to store general information (like image dimensions, image orientation, etc.)
    # Make sure the key is an integer (as needed by piexif)
    key = int(key)
...
if __name__ == '__main__':
    ...
    # Add or modify an EXIF tag
    random_key = np.random.choice(list(exif_dict['0th'].keys()))
    add_or_modify_exif_data(source_file_path, random_key, "New Value")
