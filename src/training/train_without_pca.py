...
# Train model without PCA and directly on encoded features
def train_classifier_without_pca(imports_data, function_calls_data, strings_data, labels):
    ...
    return model, encoder_imports, encoder_calls, vectorizer_strings, X


# Iteratively exclude important features
def iterative_feature_exclusion(imports_data, function_calls_data, strings_data, labels, num_iterations=10, top_n=1):
    # Initial training without PCA
    model, encoder_imports, encoder_calls, vectorizer_strings, X = train_classifier_without_pca(
        imports_data, function_calls_data, strings_data, labels
    )

    # Convert X to a dense NumPy array if it's still sparse
    ...
    for iteration in range(num_iterations):
        ...
        # Update X by removing the most important features
        X = np.delete(X, top_features_to_exclude_indices, axis=1)

        print(f"Updated X shape: {X.shape}")

        # Train the model again with the updated X
        X_train, X_test, y_train, y_test = train_test_split(X, labels, test_size=0.3, stratify=labels, random_state=42)
        model = DecisionTreeClassifier(max_depth=10, min_samples_leaf=5, random_state=42)
        model.fit(X_train, y_train)
        y_pred = model.predict(X_test)
